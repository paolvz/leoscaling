...
{   
    ...
    {   MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank - 1, MPI_MODE_NOCHECK, ghost_down_win);
        MPI_Put(first_row_point, N, MPI_DOUBLE, rank - 1, 0, N, MPI_DOUBLE, ghost_down_win);
        MPI_Win_unlock(rank - 1, ghost_down_win);
        
    }
    if (rank != size - 1){
        MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank + 1, MPI_MODE_NOCHECK, ghost_up_win);
        MPI_Put(last_row_point, N, MPI_DOUBLE, rank + 1, 0, N, MPI_DOUBLE, ghost_up_win);
        MPI_Win_unlock(rank + 1, ghost_up_win);
