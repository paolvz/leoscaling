...
touch ./OUT_${MATRIX_SIZE}_${ITERATIONS}_${METHOD}/gen_data.sh
   
cat <<EOF > ./OUT_${MATRIX_SIZE}_${ITERATIONS}_${METHOD}/gen_data.sh
#!/bin/bash
...
rm -f avg_*.out
rm -f OUT_${MATRIX_SIZE}_${ITERATIONS}_${METHOD}_*.dat

gnuplot plot_data.plt
EOF

cat <<EOF > ./OUT_${MATRIX_SIZE}_${ITERATIONS}_${METHOD}/average.py
import pandas 
...
dfs = []
for i in range(1, 6):
    dfs.append(pandas.read_csv(f"OUT_${MATRIX_SIZE}_${ITERATIONS}_${METHOD}_{i}.dat", sep=" "))
...
avg.to_csv("OUT_${MATRIX_SIZE}_${ITERATIONS}_${METHOD}.dat", sep=" ", index=False, float_format='%f')
...
touch ./OUT_${MATRIX_SIZE}_${ITERATIONS}_${METHOD}/plot_data.plt

cat <<EOF > ./OUT_${MATRIX_SIZE}_${ITERATIONS}_${METHOD}/plot_data.plt
