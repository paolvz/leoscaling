...
void mat_mul(double*A, double*B, double*C, int n_loc, int n_col, int N, int lda, int ldb, int ldc)
{   
...
int main(int argc, char **argv)
{
    ...
    for (int k = 0; k < size; k++)
    {
        ...
        mat_mul(A_LOC, B_TEMP_N, C_LOC, N_LOC, N_COL, N, N, N_COL, N);
    ...
    free(B_TEMP_N);
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        double start_comp = MPI_Wtime();
        mat_mul(A_LOC, B_LOC, C_TEMP_N, N_LOC, N, N, N, N, N);
